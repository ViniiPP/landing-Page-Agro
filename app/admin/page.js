    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-100">
        <form onSubmit={handleLogin} className="bg-white p-8 rounded shadow-md w-96">
          ...
          <input type="email" placeholder="Email" className="w-full border p-2 mb-4 rounded placeholder-black text-black" onChange={e => setEmail(e.target.value)} />
          <input type="password" placeholder="Senha" className="w-full border p-2 mb-6 rounded placeholder-black text-black" onChange={e => setPassword(e.target.value)} />
    ...
    <div className="min-h-screen bg-gray-50 p-8">
      <div className="max-w-4xl mx-auto">
        <div className="flex justify-between items-center mb-8">
          ...
          <button onClick={() => signOut(auth)} className="flex items-center gap-2 text-red-600 font-bold"><LogOut size={18}/> Sair</button>
        </div>

        {/* Formulário de Cadastro / Edição */}
        <div className={`p-6 rounded-lg shadow-md mb-10 transition-colors ${editingId ? 'bg-blue-50 border-2 border-blue-200' : 'bg-white'}`}>
          <div className="flex justify-between items-center mb-4">
            <h2 className="text-xl font-bold text-black">
              {editingId ? 'Editar Produto' : 'Novo Produto'}
            </h2>
            {editingId && (
              <button onClick={handleCancelEdit} className="text-sm text-gray-500 flex items-center gap-1 hover:text-red-500">
          ...
          <form onSubmit={handleSave} className="grid gap-4">
            <input 
              ...
              className="border p-2 rounded w-full placeholder-black text-black" required 
            />
            <textarea 
            ...
            <div className="flex gap-4 flex-col md:flex-row">
              <select value={categoria} onChange={e => setCategoria(e.target.value)} className="border p-2 rounded text-black bg-white">
                <option value="graos">Soja em Grãos</option>
                <option value="plantada">Soja Plantada</option>
              </select>
              <div className="flex-1">
                <input type="file" onChange={e => setImagem(e.target.files[0])} className="border p-2 rounded w-full text-black bg-white" accept="image/*" />
                {editingId && !imagem && <p className="text-xs text-gray-500 mt-1 ml-1">Deixe vazio para manter a imagem atual.</p>}
              </div>
            </div>
            
            <button 
              disabled={loading} 
              className={`text-white p-3 rounded flex items-center justify-center gap-2 transition disabled:opacity-50 
        ...
        <div className="grid gap-4">
          {produtos.map(prod => (
            <div key={prod.id} className="bg-white p-4 rounded shadow flex items-center justify-between border-l-4 border-green-500">
              <div className="flex items-center gap-4">
                <img src={prod.imagemUrl} alt={prod.titulo} className="w-16 h-16 object-cover rounded bg-gray-200" />
                <div>
